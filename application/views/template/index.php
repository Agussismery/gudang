...
<body>
 ...
 title: 'Apakah anda yakin akan keluar ?', type: 'warning' , showCancelButton: true, confirmButtonColor: '#DD6B55' , confirmButtonText: 'Ya, Sign Out!', closeOnConfirm: false }, function(){ window.location.href = '<?= site_url('sign_out'); ?>'; });">Sign Out</a>
    ...
    <!-- Custom Javascript -->
    <?php
    $arr_uri = array('stok_barang', 'barang', 'pegawai', 'supplier', 'data_pembelian', 'data_penjualan', 'mutu_barang', 'input_berat', 'input_pengiriman','input_kembali');
    ...
    if (in_array(strtolower($this->uri->segment(1)), $arr_uri) && !$this->uri->segment(2)) :
        switch ($this->uri->segment(1)) {
            ...
            case 'data_penjualan':
                ...
                break;
            case 'mutu_barang':
                $file = 'ajax_mutu_barang';
                break;
            case 'input_berat':
                $file = 'ajax_berat';
                break;
            case 'input_pengiriman':
        ...
        function get_item(a) {
            ...
            $.ajax({
                ...
                success: function(obj) {
                    ...
                    $('#supplier').val(x.supplier).trigger('change');
                    $('#nobal').val(x.nobal);
                    $('#id_pembelian').val(x.id_pembelian);
                    $('#harga').val(x.harga);
                    ...
                    if (x.status == 'true') {
                        ...
                        $('#jumlahx').focus();
                        $('#btn-act').html('<div class="row"><div class="col-md-6"><button type="button" class="btn btn-success btn-sm" onclick="update_cart()">Update Barang</button></div><div class="col-md-6"><button type="submit" name="submit" class="btn btn-primary btn-sm" value="Submit"><i class="fa fa-save"></i> Simpan Data Pembelian</button></div></div>');
                    }
                }
            });
        }

        function get_item_detail(a) {
            var csrf_token = Cookies.get('csrf_cookie');

            $.ajax({
                url: "<?= site_url('get_item_detail'); ?>",
                method: "POST",
                data: {
                    id: a,
                    csrf_token: csrf_token
                ...
                success: function(obj) {
                    ...
                    $('#bruto').val(x.bruto);
                    $('#netto').val(x.netto);
                    $('#barcode').val(x.barcode_pengiriman);
                    $('#tgl_kirim').val(x.tgl_pengiriman);
                    $('#id').val(x.id);
                    if ($('#bruto').val(x.bruto) - 2 == $('#netto').val(x.netto)) {
