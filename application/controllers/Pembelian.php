            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($get_barang->num_rows() == 1) {
                    ...
                    $keranjang = array(
                        ...
                        'price'   => preg_replace('/\D/', '', $this->security->xss_clean(str_replace('.', '', $this->input->post('harga', TRUE)))),
                        'name'    => $b->nama_barang,
                        'options' => [
                                        'nobal'=> $this->security->xss_clean($this->input->post('nobal', TRUE)),
                                        'supplier'=> $this->security->xss_clean($this->input->post('supplier', TRUE)),
                                        'id_pembelian'=> $this->security->xss_clean($this->input->post('id_pembelian', TRUE))
                                        ]
                    );
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($get_item) {
                $arr = [
                    'barang' => $get_item['id'],
                    'qty' => $get_item['qty'],
                    'nobal'=>$get_item['options']['nobal'],
                    'supplier'=>$get_item['options']['supplier'],
                    'id_pembelian'=>$get_item['options']['id_pembelian'],
